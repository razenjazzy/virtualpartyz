...
class TheGem_Portfolio extends Widget_Base {
	...
	public function __construct($data = [], $args = null) {
		...
		if (!defined('THEGEM_ELEMENTOR_WIDGET_PORTFOLIO_DIR')) {
			define('THEGEM_ELEMENTOR_WIDGET_PORTFOLIO_DIR', rtrim(__DIR__, ' /\\'));
		}

		if (!defined('THEGEM_ELEMENTOR_WIDGET_PORTFOLIO_URL')) {
			define('THEGEM_ELEMENTOR_WIDGET_PORTFOLIO_URL', rtrim(plugin_dir_url(__FILE__), ' /\\'));
		}

		wp_register_style(
			'thegem-portfolio-styles',
			THEGEM_ELEMENTOR_WIDGET_PORTFOLIO_URL . '/assets/css/thegem-portfolio.css',
			array(
				'thegem-animations',
				'thegem-hovers'
			),
		...
		wp_register_script(
			'thegem-portfolio-scripts',
			THEGEM_ELEMENTOR_WIDGET_PORTFOLIO_URL . '/assets/js/thegem-portfolio.js',
			array(
				'jquery',
				'isotope-js',
		...
		wp_localize_script('thegem-portfolio-scripts', 'thegem_portfolio_ajax', $localize);
	...
	public function get_style_depends() {
		...
		return ['thegem-portfolio-styles'];
	}
	...
	protected function image_style($control) {
		...
		$control->add_responsive_control(
			...
			[
				...
				'selectors' => [
					'{{WRAPPER}} .portfolio.portfolio-grid .portfolio-item' => 'padding: calc({{SIZE}}{{UNIT}}/2) !important;',
					'{{WRAPPER}} .portfolio.portfolio-grid .portfolio-row' => 'margin: calc(-{{SIZE}}{{UNIT}}/2);',
		...
		$control->add_responsive_control(
			...
			[
				...
				'selectors' => [
					...
					'{{WRAPPER}} .portfolio.portfolio-grid.title-on-page .portfolio-item .wrap' => 'border-radius: {{TOP}}{{UNIT}} {{RIGHT}}{{UNIT}} 0 0;',
					'{{WRAPPER}} .portfolio.portfolio-grid.title-on-hover .portfolio-item .wrap' => 'border-radius: {{TOP}}{{UNIT}} {{RIGHT}}{{UNIT}} {{BOTTOM}}{{UNIT}} {{LEFT}}{{UNIT}};',
					'{{WRAPPER}} .portfolio.portfolio-grid.title-on-image .portfolio-item .wrap' => 'border-radius: {{TOP}}{{UNIT}} {{RIGHT}}{{UNIT}} {{BOTTOM}}{{UNIT}} {{LEFT}}{{UNIT}};',
				],
			]
		);
	...
	public function render() {
		...
		if ($settings['pagination_type'] == 'more' || $settings['pagination_type'] == 'scroll') {
			...
			$portfolio_loop = thegem_get_portfolio_posts($terms, $page, $items_per_page, $orderby, $order);
			if (ceil($portfolio_loop->found_posts / $items_per_page) > $page)
				$next_page = $page + 1;
			else
				$next_page = 0;
		} else {
			$portfolio_loop = thegem_get_portfolio_posts($terms, 1, -1, $orderby, $order);
		}
			...
			<div class="portfolio-preloader-wrapper">
				...
				$this->add_render_attribute(
					...
					[
						...
						'data-portfolio-uid' => $portfolio_uid,
						...
						<div class="row portfolio-row">
							...
							<?php if ($settings['columns'] != '1x'): ?>
								<div class="portfolio-item-size-container">
									<?php echo thegem_portfolio_render_item(false, $settings); ?>
								</div>
							<?php endif; ?>
						</div><!-- .row-->
						<?php
		...
		if (is_admin() && Plugin::$instance->editor->is_edit_mode()): ?>
			...
			<script type="text/javascript">
				(function ($) {
					$('.elementor-element-<?php echo $this->get_id(); ?> .portfolio-grid').initPortfoliosGrid();
...
\Elementor\Plugin::instance()->widgets_manager->register_widget_type(new TheGem_Portfolio());
